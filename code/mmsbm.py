...
class MMSB_hg(object):
    ...
    def run_vem(self, tol=1e-6, IsDiagonalOptB=False,verbose= False):
        ...
        for epoch in range(self.max_iter_vem):
            elbo_old = elbo
            self.run_e_step(estep_verbose=verbose)
            estep_elbo = self.calc_elbo()
            res_elbo_emstep = np.append(res_elbo_emstep, estep_elbo)
            self.run_m_step(IsDiagonalOptB)
            mstep_elbo = self.calc_elbo()
            res_elbo_emstep = np.append(res_elbo_emstep, mstep_elbo)
            if verbose:
                log.info(f'VEM IsDiagonalOptBinMstep: {IsDiagonalOptB}, Epoch {epoch}:estep elbo={estep_elbo} ; mstep elbo={mstep_elbo}')
                #log.info(f'Epoch {epoch}: estep elbo = {estep_elbo} ; mstep elbo = {mstep_elbo}')
            if np.any(np.isnan(self.alpha)):
                log.info("NaN detected: alpha")
                break
            elbo = self.calc_elbo()
            if np.isnan(elbo):
                log.info("NaN detected: lb")
                break
            err = abs(elbo-elbo_old)
            #if verbose:
            #    log.info(f"Epoch: {epoch}, K: {self.K}, ELBO: {elbo: .6f},  Error: {err: .6f}")
            if err < tol:
                break
            res_elbo = np.append(res_elbo,elbo)
        else:
    ...
    def run_e_step(self, estep_verbose=False):
        ...
        if estep_verbose:
            elbo = self.calc_elbo()
            print(f'*** E step:before phi update elbo is {elbo}')
        self.update_phi()
        if estep_verbose:
            elbo = self.calc_elbo()
            print(f'*** E step: after phi update, elbo is {elbo}')
        self.update_gamma()
        if estep_verbose:
            elbo = self.calc_elbo()
            print(f'*** E step: after gamma upadate elbo is {elbo}')
    ...
    def update_phi(self):
        ...
        K_power = [x for x in itertools.product(*K_range)] # a list of tuples, containing all (h0-1)-perms out of [K] with replacements.
        multi_term = binom(self.N-1, self.H-1)
        for p in range(self.N):
            rem_nodeset = [x for x in range(self.N) if x != p]  # the node set removing node p
            subset_power = list(itertools.combinations(rem_nodeset, (self.H-1))) # Omega_{h0-1, -p},  see Eq(24) in project 2
            for k in range(self.K):
                sum_phi_lk = 0  # first part  in Eq(24) in project 2
                ...
                for subset in subset_power:
                    for i in range(self.K**(self.H-1)):
                        ...
                        #if self.B[k_s] > 0 and self.B[k_s] < 1: #DEBUG
                        if self.B[k_s] > 1e-50 and self.B[k_s] < 1-1e-50:
